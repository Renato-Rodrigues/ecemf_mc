...
decade_change <- plotData %>% 
 ...
 g[["Share of electricity in final energy use"]] <- ggplot(plotData,aes(x=period,y=value)) +
  ...
  scale_alpha_manual(values = modelAlpha) +
  scale_linetype_manual(values = modelLinetype) +
  labs(title = "(a) Electricity share in final energy use", subtitle = "%") +
  scale_x_continuous(expand = c(0, 0)) +
...
## Final Energy Electricity shares per sector
  ...
  g[["Share of electricity in final energy use per sector"]] <- ggplot(plotData,aes(x=period,y=value)) +
    ...
    annotate("rect", xmin = -Inf, xmax = 2024, ymin = -Inf, ymax = Inf, fill = def$hist$areaColor, alpha = def$hist$alpha) + # historical area
    geom_hline(yintercept=0, color = "black", linewidth= def$yAxis$linewidth, alpha = def$yAxis$alpha) + # y-axis zero
    geom_line(data=plotData %>% filter(model != "median"), size=1,aes(color=model,linetype=model, alpha = model)) + # models data
    geom_point(data = plotData %>% filter(!(model %in% c("Historical", "median")), period > 2023), shape = 21, aes(colour = model), fill = "white", size = 1, stroke = 1) +
    geom_line(data=plotData %>% filter(model == "median"), size=def$median$lineSize,aes(color=model,linetype=model, alpha = model)) + # models data
    geom_point(data = plotData %>% filter(model == "median"), shape = def$median$shape, aes(colour = model, shape = model), fill = "white", size = def$median$smallPointSize, stroke = def$median$smallPointStroke) +
    theme_paper() +
    facet_wrap(~sector) +
    scale_color_manual(values = setNames(color[levels(plotData$model)],levels(plotData$model))) +
    scale_alpha_manual(values = modelAlpha) +
...
g[["Share of hydrogen in final energy use"]] <- ggplot(plotData,aes(x=period,y=value)) +
  ...
  scale_alpha_manual(values = modelAlpha) +
  scale_linetype_manual(values = modelLinetype) +
  labs(title = "(b) Hydrogen share in final energy use", subtitle = "%") +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1), breaks = seq(0, 1, by = 0.2), expand = c(0, 0)) +
  expand_limits(x = 2000) +
  coord_cartesian(clip = 'off') +
...
Figure 3. Electricity and hydrogen final energy shares. Decadal average of the annual percentage change in the median model results, expressed relative to the preceding decade, shown in blue.
...
```{r Emissions per sector, echo=FALSE, include=FALSE}
  ...
  plotData <- df %>%
    ...
    theme(axis.text.x = element_text(size = 10, angle = 90, hjust = 1, vjust = 0.5)) +
    labs(title = "Emissions per sector", subtitle = expression(paste("Mt ", CO[2]," /yr"))) +
    theme(legend.key.width = unit(0.7, 'cm'),
...
g[["Carbon Capture"]] <- ggplot(plotData,aes(x=period,y=value)) +
  ...
  scale_alpha_manual(values = modelAlpha) +
  scale_linetype_manual(values = modelLinetype) +
  labs(title = "(a) Carbon Capture", subtitle = expression(paste("Mt ", CO[2]," /yr"))) + 
  scale_x_continuous(expand = c(0, 0)) +
  coord_cartesian(clip = 'off') +
  theme(strip.text = element_blank()) +
  coord_cartesian(clip = 'off')
  ...
  g[["Carbon Capture by origin"]] <- ggplot(plotData) +
    ...
    scale_fill_manual(values = carrier_color) +
    theme(axis.text.x = element_text(size = 10, angle = 90, hjust = 1, vjust = 0.5)) +
    labs(title = "(b) Carbon Capture by origin", subtitle = expression(paste("Mt ", CO[2]," /yr"))) +
    theme(legend.key.width = unit(0.7, 'cm')) +
    coord_cartesian(clip = 'off')
...
Figure 6. Carbon capture. Decadal average of the annual percentage change in the median model results, expressed relative to the preceding decade, shown in blue.
...
Figure 7. Carbon price. Decadal average of the annual percentage change in the median model results, expressed relative to the preceding decade, shown in blue.
