...
plotData_PE$variable <- factor(peLabelMap[as.character(plotData_PE$variable)], levels = names(pe_vars))
plotData_PE$model <- factor(plotData_PE$model, levels = c("LIMES", "MEESA", "OSeMBE", as.character(unique(plotData_PE$model)[!unique(plotData_PE$model) %in% c("LIMES", "MEESA", "OSeMBE")])))
...
plotData_PE_Total$model <- factor(plotData_PE_Total$model, levels = levels(plotData_PE$model))
...
if (nrow(plotData_CC) > 0) {
  ...
  plotData_CC$model <- factor(plotData_CC$model, levels = c("LIMES", "MEESA", "OSeMBE", as.character(unique(plotData_CC$model)[!unique(plotData_CC$model) %in% c("LIMES", "MEESA", "OSeMBE")])))
  ...
  plotData_CC_Total$model <- factor(plotData_CC_Total$model, levels = levels(plotData_CC$model))
...
plotData_FEC$variable <- factor(fecLabelMap[as.character(plotData_FEC$variable)], levels = names(fe_carrier_vars))
plotData_FEC$model <- factor(plotData_FEC$model, levels = c("LIMES", "MEESA", "OSeMBE", as.character(unique(plotData_FEC$model)[!unique(plotData_FEC$model) %in% c("LIMES", "MEESA", "OSeMBE")])))
...
plotData_FEC_Total$model <- factor(plotData_FEC_Total$model, levels = levels(plotData_FEC$model))
...
plotData_FES$variable <- factor(fesLabelMap[as.character(plotData_FES$variable)], levels = names(fe_sector_vars))
plotData_FES$model <- factor(plotData_FES$model, levels = c("LIMES", "MEESA", "OSeMBE", as.character(unique(plotData_FES$model)[!unique(plotData_FES$model) %in% c("LIMES", "MEESA", "OSeMBE")])))
...
plotData_FES_Total$model <- factor(plotData_FES_Total$model, levels = levels(plotData_FES$model))
...
if (nrow(plotData_FE_SC) > 0) {
  plotData_FE_SC$sector <- factor(plotData_FE_SC$sector, levels = c("Industry", "Non-Energy Use", "Transportation", "Residential & Commercial", "Agriculture", "Other Sector"))
  plotData_FE_SC$carrier <- factor(plotData_FE_SC$carrier, levels = names(fe_carrier_colors))
  plotData_FE_SC$model <- factor(plotData_FE_SC$model, levels = c("LIMES", "MEESA", "OSeMBE", as.character(unique(plotData_FE_SC$model)[!unique(plotData_FE_SC$model) %in% c("LIMES", "MEESA", "OSeMBE")])))
  ...
  plotData_FE_SC_Total$sector <- factor(fesTotalLabelMap[as.character(plotData_FE_SC_Total$variable)], levels = levels(plotData_FE_SC$sector))
  plotData_FE_SC_Total$model <- factor(plotData_FE_SC_Total$model, levels = levels(plotData_FE_SC$model))
  ...
  g[["final_energy_sector_carrier_stacked"]] <- ggplot(plotData_FE_SC) +
    ...
    theme_paper() +
    facet_grid(sector ~ period, scales = "free_y") +
    scale_fill_manual(values = fe_carrier_colors) +
    coord_cartesian(clip = 'off') +
    theme(
      axis.text.x = element_text(size = 10, angle = 90, hjust = 1, vjust = 0.5),
