...
# Model orderings for the stacked bar charts. Primary energy, final energy and carbon
# capture are whole-system quantities, so those charts only show the full system models.
# The electricity SI charts show all ten models.
...
# both drawn in the 2020 facet. `map` maps historical variable names onto the chart
# categories; several variables may map onto the same category (they are summed), and an
# optional `weight` column allows a variable to be subtracted instead (e.g. bunkers).
histStackedBars <- function(map, histModel = "IEA") {
  if (!("weight" %in% names(map))) map$weight <- 1
  groupCols <- setdiff(names(map), c("variable", "weight"))

  perYear <- histData %>%
    filter(model == histModel, region == "EUR", period %in% 2018:2022) %>%
    mutate(variable = as.character(variable)) %>%
    select(variable, period, value) %>%
    inner_join(map, by = "variable") %>%
...
- IEA covers every category for 2018-2022. Nuclear, hydro, wind and solar are only reported at the carrier level, hence the |Electricity variables; geothermal is split into an electricity and a heat variable which stack into a single segment.
...
# IEA covers every category for 2018-2022. Nuclear, hydro, wind and solar are only
...
Historical source: IEA data 
- The IEA carriers add up exactly to the IEA final energy total including bunkers in the old historical.mif.
...
Historical source: IEA data 
- The IEA has `FE = FE|Industry + FE|Transport + FE|Buildings`, so non-energy is inside industry and bunkers inside transport. I subtract those to match the models "Final Energy (w/o bunkers)".
...
# I subtract those to match the models "Final Energy (w/o bunkers)".
# The IEA has no agriculture or other-sector split.
fe_sector_hist_map <- data.frame(
  variable = c("FE|Industry", "FE|Non-energy Use", "FE|Non-energy Use", "FE|Transport",
               "FE|Transport|Bunkers", "FE|Buildings", "FE|AFOFI", "FE|Other Sector"),
  category = c("Industry", "Industry", "Non-Energy Use", "Transportation",
...
- The IEA nests non-energy use inside industry, whereas the models report it as its own sector. I couldn't find a historical breakdown of non-energy use by carrier, so for simplicit I assumed it as feedstock liquids and subtracted from industry to match the historical bars totals. Be aware that this causes only approximated comparisons on gas and liquids carrier levels at industry and non-energy use that I would avoid/fix before showing in any publication.
...
fe_sc_hist_total_map <- data.frame(
  variable = c("FE|Industry", "FE|Non-energy Use", "FE|Buildings", "FE|Transport", "FE|Non-energy Use"),
  sector   = c("Industry", "Industry", "Residential & Commercial", "Transportation", "Non-Energy Use"),
  weight   = c(1, -1, 1, 1, 1),
  stringsAsFactors = FALSE
)
...
Historical source: Ember
- unlike the IEA, Ember reports the generation total.
- Ember has no oil, geothermal or other category, so the historical stack sits a little below its own total.
...
# generation total. It has no oil, geothermal or other category, so the historical stack
# sits a little below its own total.
se_elec_hist_map <- data.frame(
...
- Embers covers every category except geothermal (about 1 GW in the EU). No source reports a historical capacity total across 2018-2022. Eurostat stops at 2018 and the IEA WEO only starts in 2021 so the historical bars carry no total diamond.
...
# Ember again, for consistency with the generation chart. It covers every category except
# geothermal (about 1 GW in the EU). No source reports a historical capacity total across
# 2018-2022 - Eurostat stops at 2018 and the IEA WEO only starts in 2021 - so the
# historical bars carry no total diamond.
cap_elec_hist_map <- data.frame(
